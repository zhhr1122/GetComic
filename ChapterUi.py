...
class App(object):
    def __init__(self, master):
        self.t_lable = Label(master,text='                               请在下方黏贴需要抓取的漫画章节url                                               漫画标题                        漫画章节数',height =2)
        self.t_lable.pack()
        ...
        self.var = StringVar()
        self.e_entry = Entry(self.f_top, textvariable = self.var,width = 80)
        self.e_entry.pack(side = LEFT)

        self.e_entry_1 = Entry(self.f_top, textvariable=self.var, width=20)
        self.e_entry_1.pack(side=LEFT)

        self.e_entry_2 = Entry(self.f_top, textvariable=self.var, width=20)
        self.e_entry_2.pack(side=LEFT)

        self.com = Button(self.f_top, text='获取章节漫画', command=self.getText, height=2)
